...
axios.get('https://jeka-api.luabot24.repl.co/random/?apikey=ralph').then(res => {
  ...
  let callback = function () {
          api.sendMessage({
                                                body: ``,
            attachment: fs.createReadStream(__dirname + `/tmp/random.${ext}`)
          }, event.threadID, () => fs.unlinkSync(__dirname + `/tmp/random.${ext}`), event.messageID);
        };
        request(res.data.url).pipe(fs.createWriteStream(__dirname + `/tmp/random.${ext}`)).on("close", callback);
      }) .catch(err => {
